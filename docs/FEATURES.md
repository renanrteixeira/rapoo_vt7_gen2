...
- [x] Receiver pairing — **app flow done (2026-08-18, story 5-4)**: the
      "Sistema" tab has a guided, user-confirmed pairing block (confirmation
      dialog before the destructive `0xA0`/`0xA1`, own daemon thread + own
      receiver fd, never touches `BatteryMonitor`). Commands mapped 🔶
      (2026-08-16, `protocol.py` + `pairing.py`). Read-only probes in
      `python3 tools/probe.py --pair-discover`. **On-device validation
      (2026-08-17)**: VID/PID poll ✅ (`24AE/4613` via `read_eeprom`
...
The full matching flow ships as `python3 tools/probe.py --pair-run` (same gate;
`PROBE_PAIR_WINDOW` env, default 60 s) — it sends both frames and watches
